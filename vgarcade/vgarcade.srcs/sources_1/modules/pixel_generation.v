...
wire instructions_on;
wire [9:0] instructions_row, instructions_col;
instructions_rom instructions (
     .clk(clk),
     .row(instructions_row),
     .col(instructions_col),
...
assign instructions_col = x - 137;
assign instructions_row = y - 76;
assign instructions_on = (x > 137) && (x < 503) &&
                      (y > 76)   && (y < 404)  &&
                      (instructions_rgb != 12'hFFF);
    ...
    if (reset) 
        ...
        case (game_state) 
            ...
            INSTRUCTIONS: begin
                if (~video_active)
                    intermediate_rgb <= 12'h000;
                else if (instructions_on)
                    intermediate_rgb <= instructions_rgb;
                else if (LRback_on)
                    intermediate_rgb <= LRback_rgb;
                else
                    intermediate_rgb <= 12'hAAA; // gray behind instructions
            end
